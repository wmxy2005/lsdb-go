...
import { Eye, File, Film, Image as ImageIcon, Trash2, Upload } from 'lucide-react';
...
function getPreviewKind(file: FileOption): 'image' | 'video' | null {
  if (file.type === 'thumbnail' || file.type === 'image') return 'image';
  if (isVideoFile(file.name)) return 'video';
  if (!file.type && IMAGE_FILE_PATTERN.test(file.name)) return 'image';
  if (file.type === 'file' && IMAGE_FILE_PATTERN.test(file.name)) return 'image';
  return null;
}
...
export function EditResourceList({
  ...
  const handlePreviewClick =
    (file: FileOption, kind: 'image' | 'video') => (e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
      openMediaPreview({
        filename: file.name,
            ...
            return (
              ...
              >
                <div className="flex min-w-0 flex-1 items-center gap-2.5">
                  ...
                  <span
                    className="truncate font-mono text-zinc-700 dark:text-zinc-300 font-medium"
                    title={f.name}
                  >
                    {f.name}
                  </span>
                  {f.type && (
                    <Badge
                      variant="outline"
                      className="shrink-0 text-[10px] font-semibold px-2 py-0 h-5 border-border/60 bg-background/50 text-zinc-500 dark:text-zinc-400 rounded-md"
                    >
                      {getTypeLabel(f.type)}
                    </Badge>
                  )}
                ...
                <div className="flex shrink-0 items-center gap-1">
                  {previewKind && (
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      className="size-8 rounded-lg text-zinc-400 hover:text-foreground hover:bg-zinc-100 dark:hover:bg-zinc-800/60 transition-colors shrink-0"
                      onClick={handlePreviewClick(f, previewKind)}
                  ...
                  <Button
                    ...
                    aria-label={t('edit.resource.delete')}
                  >
