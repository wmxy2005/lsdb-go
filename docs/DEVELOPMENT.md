...
- 前端 `/tool` 监控无数据：确认已登录（SSE 经 `?token=` 传 JWT）；关闭监控开关或 SSE 断开后，超过 `LSDB_MONITOR_IDLE_TIMEOUT`（默认 30s）采样会停止，重新打开即可。`/speedTest` 测速失败同理检查 Token。
...
- 统一响应 `{success,data,errorCode}`；`response.Fail` 返回语义化 HTTP 状态码（历史行为曾将失败统一改写为 HTTP 202，现已废弃）；前端 `apiRequest` 解析错误 body 后仍据 `success` 判断。
...
失败时 HTTP 状态码与 `errorCode` 一致（如 401/404/409）。历史行为曾将失败统一改写为 HTTP 202，现已改为语义化状态码，请勿再依赖 2xx 判断失败。前端 `apiRequest` 会把含 `success` 的错误 JSON 正常返回，页面仍按 `success` 字段判断。
...
1. `**Table`、`Home`、`Access` 等页面**：Umi 脚手架遗留，路由仍注释；`SpeedTest` 已上线（`/speedTest`）。是否删除未启用页面待确认。
...
7. **版本与许可证（部分已确认，尚未完全统一）**：
  - 已确认：维护者统一为 `wmxy2005`；桌面端版本 `0.1.0`；前端 [frontend/LICENSE](../frontend/LICENSE) 为 Apache 2.0；UI 页脚版权为 `Copyright © 2026 By wmxy2005`；源码仓库为 `https://github.com/wmxy2005/lsdb-go`。
